...
1. Create **one or many micro-frontend apps** compatible with MfMaestro dynamic load mechanism ([demo](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/micro-app-1/app.js)) and a manifest file ([demo](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/manifest.json)), and put them on a webserver
...
3. Add pages folder with your first page ([see demo](https://gitlab.com/calions/mf-maestro/blob/master/test/src/pages/Home.js))
...
A demo application (also used for integration tests) is availabe in [the```test```directory](https://gitlab.com/calions/mf-maestro/tree/master/test)

It loads multiple micro applications, coded with different frameworks and versions (VannillaJS, Elm, Angular, Vue, React, EmberJs). The important mechanisms to look at are tested in the [```test/tests/test.js```](https://gitlab.com/calions/mf-maestro/blob/master/test/tests/test.js) file.
...
You can have a look at our 2 manifest files ([1](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/manifest.json) and [2](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/manifest2.json)) in the demo application. And look how we build a [page](https://gitlab.com/calions/mf-maestro/blob/master/test/src/pages/Home.js) : it's almost only declarative at the beginning. Simple and fast to start!
...
So, on our web server, for a microservice, we have N ```.js``` files (one per micro-frontend application, see the demo files here : [1](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/micro-app-1/app.js), [2](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/micro-app-2/app.js), [3](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/micro-app-3/app.js), [4](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/apps-react-15/app.js), [5](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/apps-react-16-8-hooks/app.js), [6](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/apps-vue/app.js)), each one registering in our main application an object with a start and a stop method when it is loaded, and 1 manifest file per service (see demo here: [1](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/manifest.json), [2](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/manifest2.json)) with metadata to tell to our MfMaestro app where to find the js and css files for our micro-frontends.
...
(1) **```components```** directory : put here your own React components for your pages. You decide to create a new component usually when you need to handle specific logic (events handling, side effects...) not available in MfMaestro basic components. You can view an [example](https://gitlab.com/calions/mf-maestro/blob/master/test/src/components/users/All.js).

(2) **```components/users```** directory : this directory contains a component with more logic : [see an example](https://gitlab.com/calions/mf-maestro/blob/master/test/src/components/users/All.js)

(3) **```index.js```** : this file starts the app ([see an example](https://gitlab.com/calions/mf-maestro/blob/master/test/src/index.js)) calling [the ```startMediator```function](https://gitlab.com/calions/mf-maestro/blob/master/src/startMediator.js) with 3 args :

- **targetDomElementId** : the id of the dom element where we want to start our app
- **MainPage** : the root component of your app, injected into the [React router](https://gitlab.com/calions/mf-maestro/blob/master/src/MediatorApp.js)
- **init** : a function call at the mediator start to configure the application, see bellow init.js (4)

(4) **```init.js```** : this file exports a function (run when [the main app starts](https://gitlab.com/calions/mf-maestro/blob/master/src/MediatorApp.js#L10)) used mainly to define how [our events are managed](https://) and config options ([see demo file](https://gitlab.com/calions/mf-maestro/blob/master/test/src/init.js)).
The object returned by this function is also used as a [config for our MediatorApp](https://gitlab.com/calions/mf-maestro/blob/master/src/services/configureApp.js). For now you can only pass these keys :

- **MicroAppLoadingComponent** : a component to replace the default loading component (what is visible on the page in the micro-frontend destination div while the mediator loads a manifest or a js code file : [default](https://gitlab.com/calions/mf-maestro/blob/master/src/MicroAppLoadingComponent.js) and [custom for demo app](https://gitlab.com/calions/mf-maestro/blob/master/test/src/components/DemoMicroAppLoadingComponent.js).

(5) **```pages```** directory : all app's pages where we put components/micro-frontends.

(6) **```MainPage.js```** : the main page of our app ([see example](https://gitlab.com/calions/mf-maestro/blob/master/test/src/pages/MainPage.js)) injected in the ```startMediator```function in ```index.js``` file (3). Usually this page would be your main router.  

### Options and params sent [to start() function](https://gitlab.com/calions/mf-maestro/blob/master/src/MicroAppTypes/NativeMicroApp.js) of your micro-frontend

1. microAppId

 This is the id of the html node where you can find the "app-wrapper" node of your app (the node where your app is injected). You should not need it most of the time. It is node the node where your app starts, but [its parent](https://gitlab.com/calions/mf-maestro/blob/master/src/MicroAppTypes/NativeMicroApp.js)
...
### How to use query params
  ...
  and be able to send these values in your [micro-frontends ```start``` functions](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/micro-app-1/app.js).

  To let the mediator connect your url's json5 to a micro-frontend on the page, you need to add a ```groupRef``` prop to your react MicroAppComponent component ([demo](https://gitlab.com/calions/mf-maestro/blob/master/test/src/pages/Home.js)), and use this groupRef as the key of your json5 value in your query string (```home1``` here): ```...myUrl?home1={my json5 object}```
...
You can view some examples [in the micro app 2 demo code](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/micro-app-2/app.js).
...
In the options passed to the ```start```function, you have [```options.navigation```](#options-navigation). You find in this object [2 functions](https://gitlab.com/calions/mf-maestro/blob/master/src/navigation.js) you can call when you need to block or authorize navigation. This can be usefull for example, if you need to show a modal to the user for validation before leaving a page.
...
You can view a [demo here](https://gitlab.com/calions/mf-maestro/blob/master/test/public/assets/micro-app-2/app.js#L20)
